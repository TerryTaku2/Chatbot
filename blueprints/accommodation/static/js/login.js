...
loginForm.addEventListener('submit', async (e) => {
  ...
  try {
    const res = await fetch('/accommodation/login', {
      method: 'POST',
    ...
    if (res.ok && data.success) {
      ...
      setTimeout(() => { window.location.href = data.redirect || '/accommodation/dashboard'; }, 600);
...
regForm.addEventListener('submit', async (e) => {
  ...
  try {
    const res  = await fetch('/accommodation/register', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (res.ok && data.success) {
      ...
      setTimeout(() => { window.location.href = data.redirect || '/accommodation/dashboard'; }, 800);
    } else {
