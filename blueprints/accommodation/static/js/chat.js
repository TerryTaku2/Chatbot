...
async function loadConversations() {
  try {
    const res  = await fetch('/accommodation/api/conversations');
...
async function openConversation(convId) {
  ...
  if (conv) {
    ...
    if (conv.property_id && conv.property_title) {
      ...
      threadPropLink.href           = `/accommodation/landlord/property/${conv.property_id}`;
  ...
  try {
    const res  = await fetch(`/accommodation/api/messages/${convId}`);
...
function markRead(convId) {
  fetch(`/accommodation/api/conversations/${convId}/read`, { method: 'POST' }).catch(() => {});
}
...
window.startConversation = async function(recipientId, propertyId, propertyTitle) {
  try {
    const res = await fetch('/accommodation/api/conversations/start', {
      method: 'POST',
    ...
    if (data.conv_id) window.location.href = `/accommodation/messages?c=${data.conv_id}`;
...
function refreshUnreadBadge() {
  fetch('/accommodation/api/messages/unread-count')
    .then(r => r.json())
    .then(({ count }) => {
